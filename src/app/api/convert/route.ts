// src/app/api/convert/route.ts — NanoBanana API
...
async function pollTask(taskId: string, apiKey: string): Promise<string> {
  for (let i = 0; i < 40; i++) {
    ...
    if (flag === 2 || flag === 3) throw new Error(`Convert failed: ${json?.data?.errorMessage}`);
    if (flag === 1) {
...
export async function POST(req: Request) {
  try {
    ...
    const verticalPrompt = `${originalPrompt.slice(0, 2700)} 9:16 vertical Stories format.`.slice(0, 2800);
    ...
    return NextResponse.json({ id: Math.random().toString(36).slice(2), url: imageUrl, prompt: verticalPrompt });
